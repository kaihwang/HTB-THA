...
for s in 041 044 048 054 062 067 070 077 080 084 042 045 049 057 063 068 071 078 082 043 046 053 059 064 069 076 079 083; do 
	...
	for task in HTB; do
		...
		for f in $(ls ${Raw}/func/sub-${s}_task-${task}run*confounds.tsv | sort -V); do
			cat ${f} | tail -n+2 | cut -f1-2,14-19 >> ${Output}/sub-${s}/confounds.1D
		...
		if [ -e ${Output}/sub-${s}/censor.1D ]; then
			...
			-stim_times 1 /data/backed_up/kahwang/HTB/fMRIprep/DesignMatrices/sub-${s}_R4.1D 'SPMG3(2)' -stim_label 1 R4 \
			-stim_times 2 /data/backed_up/kahwang/HTB/fMRIprep/DesignMatrices/sub-${s}_R8.1D 'SPMG3(2)' -stim_label 2 R8 \
			-stim_times 3 /data/backed_up/kahwang/HTB/fMRIprep/DesignMatrices/sub-${s}_D1.1D 'SPMG3(2)' -stim_label 3 D1 \
			-stim_times 4 /data/backed_up/kahwang/HTB/fMRIprep/DesignMatrices/sub-${s}_D2.1D 'SPMG3(2)' -stim_label 4 D2 \
			...
			-num_glt 16 \
			-gltsym 'SYM: +1*D1[0] +1*D2[0] -1*R4[0] -1*R8[0] ' -glt_label 1 Abstraction \
			-gltsym 'SYM: +1*D2[0] +1*R8[0] -1*D1[0] -1*R4[0] ' -glt_label 2 Load \
			-gltsym 'SYM: +1*D2[0] -1*D1[0] -1*R8[0] +1*R4[0] ' -glt_label 3 Interaction \
			-gltsym 'SYM: +1*R8[0] -1*R4[0] ' -glt_label 4 R8-R4 \
			-gltsym 'SYM: +1*D2[0] -1*D1[0] ' -glt_label 5 D2-D1 \
			-gltsym 'SYM: +1*D2[0] -1*R8[0] ' -glt_label 6 D2-R8 \
			-gltsym 'SYM: +1*D1[0] -1*R4[0] ' -glt_label 7 D1-R4 \
			-gltsym 'SYM: +1*D2[0] -1*R4[0] ' -glt_label 8 D2-R4 \
			-gltsym 'SYM: +1*R8[0] -1*D1[0] ' -glt_label 9 R8-D1 \
			-gltsym 'SYM: +0.5*R4[0] +0.5*R8[0] ' -glt_label 10 Response \
			-gltsym 'SYM: +0.5*D1[0] +0.5*D2[0] ' -glt_label 11 Dimension \
			-gltsym 'SYM: +1*D1[0] ' -glt_label 12 D1 \
			-gltsym 'SYM: +1*D2[0] ' -glt_label 13 D2 \
			-gltsym 'SYM: +1*R4[0] ' -glt_label 14 R4 \
			-gltsym 'SYM: +1*R8[0] ' -glt_label 15 R8 \
			-gltsym 'SYM: +1*D1[0] +1*D2[0] +1*R4[0] +1*R8[0] ' -glt_label 16 alltask \
			-rout \
			-tout \
			...
			#dont need FDR curve
			sed -i 's/-verb/-noFDR -verb/g' /data/backed_up/kahwang/HTB/fMRIprep/Results/sub-${s}/FIRmodel_task-HTB_MNI_stats.REML_cmd
