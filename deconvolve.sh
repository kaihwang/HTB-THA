...
for s in 041; do  #sub43 sub44 sub45 sub46 sub48 sub49 sub52 sub53 sub54
	...
	for task in HTB; do
		...
		for run in 1 2 3 4 5 6 7 8; do
			...
			if [ ! -e ${Raw}/func/sub-${s}_task-${task}run${run}_bold_space-MNI152NLin2009cAsym_preproc_scaled.nii.gz ]; then
				
				# No spatial smoothing. Commented out
				...
				3dcalc \
				-a ${Raw}/func/sub-${s}_task-${task}run${run}_bold_space-MNI152NLin2009cAsym_preproc.nii.gz \
				...
				-expr "(a/b * 100) * c" \
				-prefix ${Raw}/func/sub-${s}_task-${task}run${run}_bold_space-MNI152NLin2009cAsym_preproc_scaled.nii.gz
		...
		for f in $(ls ${Raw}/func/sub-${s}_task-${task}run*confounds.tsv | sort -V); do
			cat ${f} | tail -n+2 | cut -f14-19 >> ${Output}/sub-${s}/confounds.tsv
			cat ${f} | tail -n+2 | cut -f22-27 >> ${Output}/sub-${s}/motion.tsv
		done
		...
		1d_tool.py -infile ${Output}/sub-${s}/motion.tsv -set_nruns 8 -derivative -censor_prev_TR -collapse_cols euclidean_norm \
        -moderate_mask -1.2 1.2            \
        -show_censor_count                 \
        -write_censor ${Output}/sub-${s}/censor.1D
		
		1d_tool.py -infile ${Output}/sub-${s}/motion.tsv -set_nruns 8 -overwrite -derivative -write ${Output}/sub-${s}/motion.deriv.tsv 
		...
		if [ -e ${Output}/sub-${s}/censor.1D ]; then
			...
			-ortvec ${Output}/sub-${s}/motion.tsv motion \
			-ortvec ${Output}/sub-${s}/motion.deriv.tsv motion_deriv \
			-stim_times 1 /data/backed_up/kahwang/HTB/fMRIprep/DesignMatrices/sub-${s}_R4.1D 'TENT(0, 15, 11)' -stim_label 1 R4 \
			-stim_times 2 /data/backed_up/kahwang/HTB/fMRIprep/DesignMatrices/sub-${s}_R8.1D 'TENT(0, 15, 11)' -stim_label 2 R8 \
			-stim_times 3 /data/backed_up/kahwang/HTB/fMRIprep/DesignMatrices/sub-${s}_D1.1D 'TENT(0, 15, 11)' -stim_label 3 D1 \
			-stim_times 4 /data/backed_up/kahwang/HTB/fMRIprep/DesignMatrices/sub-${s}_D2.1D 'TENT(0, 15, 11)' -stim_label 4 D2 \
			...
			-num_glt 16 \
			-gltsym 'SYM: +1*D1[2..7] +1*D2[2..7]2 -1*R4[2..7] -1*R8[2..7] ' -glt_label 1 Dimension \
			-gltsym 'SYM: +1*D2[2..7] +1*R8[2..7] -1*D1[2..7] -1*R4[2..7] ' -glt_label 2 Load \
			-gltsym 'SYM: +1*D2[2..7] -1*D1[2..7] -1*R8[2..7] +1*R4[2..7] ' -glt_label 3 Interaction \
			...
			-gltsym 'SYM: +1*D1[2..7] +1*D2[2..7] +1*R4[2..7] +1*R8[2..7] ' -glt_label 16 alltask \
			-rout \
			-tout \
